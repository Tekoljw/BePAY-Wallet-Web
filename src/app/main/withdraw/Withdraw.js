                            ...
                            <div id="GoogleCodeSty" className="PINSty">
                                <div className='pinWindow'>
                                    ...
                                    { typeBinded ? ( (twiceVerifyType == 0 ||  twiceVerifyType == 1 ) ? 
                                        <div className='mt-16' style={{ fontSize:"16px",textAlign:"center" }}> {t('kyc_59')}{ twiceVerifyType ===0 ? `邮箱 ${userData?.userInfo?.email}` : `手机号 ${ '+' + userData?.userInfo?.nation + userData?.userInfo?.phone}`} <span style={{ color:"#2dd4bf", textDecoration:"underline"}}  onClick={ ()=> reciveCode()}>接收</span> 
                                        </div>: <div className='mt-16' style={{ fontSize:"16px",textAlign:"center" }}> 请在google验证器查看</div>)
                                     : <div className='mt-16' style={{ fontSize:"16px",textAlign:"center" }}> 您还没有绑定{ twiceVerifyType ===0 ? '邮箱' :  twiceVerifyType ===1 ? '手机号': 'Google验证' } <span style={{ color:"#2dd4bf", textDecoration:"underline" }} onClick={ ()=> bindTwiceVerifyType()} >{t('card_167')}</span> </div>
                                    }
